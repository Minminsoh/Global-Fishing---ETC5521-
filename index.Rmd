...
The 10 countries with the highest average consumption from 1961 to 2017 are summarised in the table \@ref(tab:top10tbl) while table \@ref(tab:low10tbl) contains the list of 15 countries with the lowest average consumption.  On islands with little agricultural land, fish are frequently the primary source of protein, contributing in some small island states to over half of the animal protein intake   
...
First, we are interested in finding out whether the distribution of consumption from 1961 to 2017 among the 15 countries with the highest consumption are the same. Then, we will investigate the same for the 15 countries with the average lowest consumption across years. 
...
```{r top-distplot, fig.cap=" Box plot and Density plot showing the distribution of average consumption per person per capita (kg) across years among the 15 countries with the highest consumption.", echo=FALSE}
...
Figures \@ref(fig:top-distplot) illustrates the seafood distribution per capita from 1961 to 2017. Maldives is the country with the widest dispersion of seafood consumption over time ranging from 13.19kg per person per year to 191.75kg per person per year. Countries such as Japan, Kiribati and Norway are not dispersed, which means that the average seafood consumption over time do not vary much as compared to the countries. 

It's worth noting that Portugal's boxplot displays some outliers falling outside the quartiles, where one of the year the average consumption is significantly higher  at 71.48kg per person. 

```{r}
# Ridge line plot

low1 <- ggplot(low10_consumption, 
       aes(x = fct_reorder(Entity, consumption, .fun = median),  #.fun? 
           y = consumption)) +
  geom_boxplot() + 
  coord_flip()+
...
### Geographic Differences : Trend in seafood consumption for top 10 nations & lowest 10 nations 
...
```{r countryconsumption1,  echo=FALSE, preserve = TRUE, fig.pos = 'H', out.extra = "", fig.cap = "Seafood consumption among the top 10 countries over time.  This plot is the same as previous plot but it allows interative plot elements. "}
ggplotly(top3)  
...
top4 <- ggplot(data = top10_consumption,
  ...
  scale_x_continuous(breaks = seq(min(top15_consumption$Year), max(top15_consumption$Year), by=15))+
  scale_y_continuous(breaks = seq(min(top15_consumption$consumption), max(top15_consumption$consumption), by=30))+
  facet_wrap(~Entity)
